# -*- coding: utf-8 -*-
"""Тест качества ТРАНСКРИПЦИИ (ru/tt) — оценочный прогон на наборе аудио.

ЧТО ЭТО. Не pytest-юнит-тест, а оценочный тест-риг (end-to-end / integration):
прогоняет реальный продакшен-путь транскрипции на папке аудиофайлов и сохраняет
результат, чтобы проверить КАЧЕСТВО распознавания сверкой с эталоном.
(Будущие тесты других вещей — авторизация, загрузка и т.п. — отдельные файлы.)

ЧТО ПРОВЕРЯЕТ (quality requirements транскрипции):
  - распознавание речи (ru через Whisper-large-v3, tt через Whisper-Tatar);
  - корректность пословного тега языка ru/tt (спецбуквы + словарь + аудио-LID);
  - разбивку на предложения/говорящих (sentences) при включённой диаризации.

КАК. Использует ТЕ ЖЕ модули, что и прод (backend/src), т.е. тестирует ровно тот
код, что работает в API: VAD -> аудио-LID (mms-lid) -> маршрут tt:Whisper-Tatar /
ru:Whisper-large-v3 -> пословный тег. На каждый файл пишет в OUT_DIR:
  <name>.json — слова с таймкодами, тегами ru/tt, (опц.) говорящими + sentences;
  <name>.txt  — плоский транскрипт.

ВХОД: папка с аудио (SRC_ROOT). ВЫХОД: OUT_DIR/<name>.json|.txt — сверяются с
ожидаемой (эталонной) разметкой ru/tt.

ВНЕШНИЕ ЗАВИСИМОСТИ: внешних API НЕТ (Claude/LLM/сеть в рантайме НЕ нужны). Нужны
только локальные ML-модели (Whisper, mms-lid, ECAPA) — скачиваются с HuggingFace
ОДИН раз при первом запуске и кэшируются, дальше работает офлайн. Плюс локальные
аудиофайлы. Тяжело по CPU/RAM (см. docs/ru_tt_pipeline.md), но самодостаточно.

Запуск:
  python scripts/transcription_quality_test.py            # все файлы из SRC_ROOT
  python scripts/transcription_quality_test.py <substr>   # только файлы с substr в имени
"""
